'''
Modified from: http://dtmoodie.blogspot.cz/2015/02/getting-image-watch-like-debugging-in.html
'''
...
class ImshowCommand(gdb.Command):
    def __init__(self):
        super(ImshowCommand, self).__init__("imshow",
                                             gdb.COMMAND_DATA,
                                             gdb.COMPLETE_SYMBOL)
    def invoke(self, arg, from_tty):
        ...
        # allows one to type "imshow someimage" in the GDB commandline
        ...
        data_address = str(v['data']).encode('utf-8').split()[0]
        ...
        # otherwise, we would use memory across processes       
        print("Rows:", int(v['rows']))
        buf = v['step']['buf']
        print("buf:", int(buf[0]), int(buf[1]))
        bytes = int(buf[0]) * int(v['rows']) # buf[0] is the byte stride?
        ...
        if(depth == CV_16U):
                print('16bit image')
                img = numpy.frombuffer(mem, count=bytes/2, dtype=numpy.uint16)
        if(depth == CV_32F):
                print('32bit float image')
                print(int(bytes))
                img = numpy.frombuffer(mem, count=round(bytes/4), dtype=numpy.float32)
        print("From buffer done")
        img = img.reshape((int(v['rows']), int(v['cols']), channel))
        print(img.shape)
